...
public class SocialAccountController {
	...
	@PostMapping("/account")
	public ResponseEntity<?> createAccount(@Valid @RequestBody SocialAccount account) throws SameUserAndPlatformException {
		...
		Optional<SocialAccount> foundPlatform = repo.findByPlatform(account.getPlatform());
		
		// make sure each account created has a unique account name or platform , if not checked, will end up with 409 error
	...
	//create account throws sameUser exception
		@PutMapping("/account")
		public ResponseEntity<?> updateAccount(@Valid @RequestBody SocialAccount account) throws ResourceNotFoundException, SameUserAndPlatformException {
