...
public class JwtRequestFilter extends OncePerRequestFilter {
	...
	// check the header of the request and make sure the jwt is valid in order to
	// access the API its requesting
	@Override
	protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain)
		...
		// security context -> stores info on the currently authenticated users (users
		// who have generated a jwt
		...
		if (username != null && SecurityContextHolder.getContext().getAuthentication() == null) {
			...
			// check if token is valid based on the given user and as long as token is not
			// expired
			if (jwtUtil.validateToken(jwt, userDetails)) {

				// code below is what is usually done by default by spring security
				// but b/c we have our own token-based authentication, we have to place our
				// tokens in security context
		...
		// since there may be more filters that will need to access the request, we
		// finish up and pass along
